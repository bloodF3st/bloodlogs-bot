...
Время в логах — `message.date` из Telegram API (точное серверное время отправки). Лог может прийти в канал с небольшой задержкой из-за очереди relay, но время в тексте всегда оригинальное.
